...
//functions to export
extern "C" _declspec(dllexport) int SetKeyboardHook();
extern "C" _declspec(dllexport) int UnSetKeyboardHook();
extern "C" _declspec(dllexport) LRESULT CALLBACK KeyboardProc(int nCode, WPARAM wParam, LPARAM lParam);
//_declspec(dllexport) BOOL CALLBACK InputIsEmpty();
//_declspec(dllexport) std::string CALLBACK GetInput();
